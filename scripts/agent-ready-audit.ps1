...
function Validate-BundlesScopes {
  ...
  foreach ($bundle in $Bundles.bundles.PSObject.Properties.Value) {
...
function Validate-BundlesIntentCoverage {
  ...
  $covered = @($Bundles.bundles.PSObject.Properties.Value | ForEach-Object { [string]$_.intent })
  $missing = New-Object System.Collections.Generic.List[string]
...
$wikiPath = [string]$defaults.wikiPath
...
try {
  $out = pwsh scripts/whatfirst-lint.ps1 -OrchDir $orchestrationsDir -IntentsDir $intentsDir -SummaryOut $tmpWhat | Out-String
...
foreach ($scopeName in $scopeSet) {
  ...
  try {
    $fm = pwsh scripts/wiki-frontmatter-lint.ps1 -Path $wikiPath -ExcludePaths logs/reports -ScopesPath $scopesPath -ScopeName $scopeName -DraftHygiene warn -SummaryOut $tmpFm | Out-String
    ...
    $tags = pwsh scripts/wiki-tags-lint.ps1 -Path $wikiPath -TaxonomyPath $taxonomyPath -ExcludePaths logs/reports -RequireFacets -RequireFacetsScope core -ScopeName $scopeName -SummaryOut $tmpTags | Out-String
    ...
    $links = pwsh scripts/wiki-links-anchors-lint.ps1 -Path $wikiPath -ExcludePaths logs/reports,old,.attachments -ScopesPath $scopesPath -ScopeName $scopeName -SummaryOut $tmpLinks | Out-String
    ...
    $sum = pwsh scripts/wiki-summary-lint.ps1 -Path $wikiPath -ExcludePaths logs/reports,old,.attachments -ScopesPath $scopesPath -ScopeName $scopeName -SummaryOut $tmpSummary | Out-String
