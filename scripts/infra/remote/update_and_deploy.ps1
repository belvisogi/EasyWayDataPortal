...
Write-Host "🚀 Avvio Update & Deploy su $IP..."
...
chmod +x scripts/ci/deploy-local.sh
chmod +x scripts/infra/setup-easyway-server.sh

echo "🚀 [3/3] Esecuzione Deploy Atomico..."
...
if ($WhatIf) {
    ...
    Write-Host "WHATIF: would run deploy-local.sh" -ForegroundColor Yellow
    return
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "`n❌ ERRORE DEPLOY" -ForegroundColor Red
}
