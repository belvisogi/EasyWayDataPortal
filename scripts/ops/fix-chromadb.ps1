...
Write-Host "🔍 ChromaDB Deep Diagnostic on $TargetIP..." -ForegroundColor Red
Write-Host "="*60
Write-Host ""

# 1. Check ALL containers (running AND stopped)
Write-Host "[1] All Docker Containers:" -ForegroundColor Yellow
ssh -i $KeyPath -o StrictHostKeyChecking=no ubuntu@$TargetIP "sudo docker ps -a --format 'table {{.Names}}\t{{.Status}}\t{{.Image}}'"
...
ssh -i $KeyPath -o StrictHostKeyChecking=no ubuntu@$TargetIP "sudo docker logs --tail 20 chromadb 2>&1 || echo 'Container does not exist or has no logs'"

Write-Host ""

# 3. Check if docker-compose.yml exists and what it contains
Write-Host "[3] Docker Compose File Status:" -ForegroundColor Yellow
ssh -i $KeyPath -o StrictHostKeyChecking=no ubuntu@$TargetIP "cd ~/EasyWayDataPortal && ls -lh docker-compose.yml && grep -A 3 'chromadb:' docker-compose.yml 2>&1 || echo 'File not found'"
...
$startResult = ssh -i $KeyPath -o StrictHostKeyChecking=no ubuntu@$TargetIP "cd ~/EasyWayDataPortal && sudo docker compose up -d chromadb 2>&1"
...
ssh -i $KeyPath -o StrictHostKeyChecking=no ubuntu@$TargetIP "sudo docker ps --filter 'name=chromadb' --format 'table {{.Names}}\t{{.Status}}'"
