...
IF OBJECT_ID('PORTAL.fn_rls_tenant_filter','IF') IS NULL
EXEC('CREATE FUNCTION PORTAL.fn_rls_tenant_filter(@tenant_id NVARCHAR(50)) RETURNS TABLE AS RETURN SELECT 1 AS fn_result');
GO

/* Redefine body (idempotent via CREATE OR ALTER not supported on IF) */
ALTER FUNCTION PORTAL.fn_rls_tenant_filter(@tenant_id NVARCHAR(50))
RETURNS TABLE
AS
RETURN (
